...
public class MqManager {
    ...
    public void createQuBot(String name) {
        try {
            botEvent = session.createQueue(name);
            botEventProducer = session.createProducer(botEvent);
        } catch (JMSException e) {
            e.printStackTrace();
        }
    }
    ...
    public void botProducer(String message) {
        try {
            TextMessage textMessage = session.createTextMessage(message);
            botEventProducer.send(textMessage);
        } catch (JMSException e) {
            e.printStackTrace();
        }
    }
